...
def get_notion_data():
    ...
    all_semesters = [row["properties"]["학기"]["select"]["name"] for row in results if row["properties"].get("학기")]
    ...
    for row in results:
        ...
        try:
            ...
            page_url = row.get("url") # 노션 페이지 고유 URL 가져오기
            ...
            # 박스를 클릭 가능한 <a> 태그로 감쌉니다.
            box = f'''
            <a href="{page_url}" target="_blank" class="class-link">
                <div class="class-box {color}">{name}</div>
            </a>
            '''
            for day in days:
